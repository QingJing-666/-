### 本文由B站up主算法工程师01所出，请勿用于商业行为

具体讲解链接：https://www.bilibili.com/video/BV18s421M7pZ/?spm_id_from=333.999.0.0

### SENet通道注意力机制

SE-Net通过引入通道注意力机制，使模型能够更好地捕捉到不同通道的重要性。传统卷积网络会把所有通道平等对待，而SE-Net则尝试通过学习一个权重来强调某些更有信息量的通道，同时抑制不重要的通道。
...
ECA是改进的通道注意力机制，在不显著增加计算开销的前提下，利用更加简洁的方式来捕捉通道间的依赖关系，从而提升卷积神经网络的性能。
...
if __name__ == "__main__":
    x = torch.randn(4, 64, 32, 32)
    eca = ECALayer(in_channels=64, k_size=3)
    y = eca(x)
    print(y.shape)  # 输出: torch.Size([4, 64, 32, 32])
```
...
CBAM同时关注空间和通道两个方面的特征，能够更精确地捕捉重要信息。
...
SKNet的核心创新在于Selective Kernel (SK)模块，允许网络在多个卷积核之间选择，提供了自适应感受野和增强特征表示的优势。

### SPANet

SPAnet结合了SPP的多尺度特征捕捉能力和注意力机制的自适应权重调整，提高了模型在处理多尺度和复杂特征图像时的性能表现。
